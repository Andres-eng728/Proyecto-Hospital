...
public class Persistencia {
     ...
     * @throws IOException
     */
    public static void serializarObjetoMedico(String ruta, Object objeto) throws IOException {
        File archivo = new File(ruta);
        archivo.getParentFile().mkdirs(); // Crea los directorios si no existen

        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("Data\\BaseDatosMedicos.txt"))) {
            oos.writeObject(objeto);
        }
    }
     ...
     * @throws Exception
     */
    public static Object deserializarObjetoMedico(String ruta) throws IOException, ClassNotFoundException {
        File archivo = new File(ruta);
        if (!archivo.exists()) {
            return null;
        }

        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("Data\\BaseDatosMedicos.txt"))) {
            return ois.readObject();
        }
    }
